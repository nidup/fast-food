...
var EASYSTAR_PATH = './node_modules/easystarjs/bin/';
 ...
 * This way you can call 'npm update', get the lastest Phaser version and use it on your project with ease.
 */
function copyEasyStar() {

    var srcList = ['easystar-0.3.0.min.js'];

    if (!isProduction()) {
        srcList.push('easystar-0.3.0.js');
    }

    srcList = srcList.map(function(file) {
        return EASYSTAR_PATH + file;
    });
...
gulp.task('copyEasyStar', ['copyPhaser'], copyEasyStar);
gulp.task('build', ['copyEasyStar'], build);
 ...
 * 'cleanBuild' -> 'copyStatic' -> 'copyPhaser' -> 'copyEasyStar' -> 'build' -> 'serve'
 * 
